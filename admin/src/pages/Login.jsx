...
const Login = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/admin/auth`, {
  ...
  const handleLoginSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/admin/login`,
        { name, email, password },
        { withCredentials: true }
      );
      if (data.success) {
  ...
  if (loading) return <p className="text-white">Checking login...</p>;

  return (
    <section className="w-full min-h-screen bg-[#1a1a1d] flex flex-col items-center
     justify-center text-white px-6 py-12">
      {/*Header */}
        ...
        <form onSubmit={handleLoginSubmit} className="space-y-5">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl
              bg-pink-600 hover:bg-pink-700 shadow-lg shadow-pink-800/40
